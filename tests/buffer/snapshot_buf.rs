...
fn append_consume_grow_and_enforce_capacity() {
    let mut buf = SnapshotBuf::<{ 1 << 20 }>::try_with_capacity(16 * 1024).unwrap();
    assert!(buf.is_empty());
    ...
    assert!(!buf.try_advance(usize::MAX));
    assert_eq!(buf.len(), 11);

    assert!(buf.try_advance(6));
    ...
    assert!(buf.try_advance(buf.len()));
    buf.compact();
    assert!(buf.is_empty());
    assert!(buf.snapshot().is_none());

    let mut small = SnapshotBuf::<{ 16 * 1024 }>::try_with_capacity(16 * 1024).unwrap();
    assert!(
        small
