use o3::collections::slab::{Capacity, Cell, Exclusive, key::Parts};
...
fn generational_reuse_and_capacity() {
    let mut slab: Exclusive<&str> = Exclusive::with_capacity(Capacity::new(3));
...
fn entry_and_explicit_index_paths_preserve_the_free_list() {
    let mut slab = Exclusive::<u32>::with_capacity(Capacity::new(200));
...
fn range_reservation_never_escapes_its_partition() {
    let mut slab = Exclusive::<u32>::with_capacity(Capacity::new(6));
    let occupied = slab.slots().vacant_entry_at(2).unwrap().insert(2);
    let occupied_next = slab.slots().vacant_entry_at(3).unwrap().insert(3);

    let selected = slab
        .slots()
        .vacant_entry_in(2..5)
        .expect("free slot in range");
    assert_eq!(selected.key().index(), 4);
    let selected = selected.insert(4);
    assert_eq!(slab.get(selected), Some(&4));

    assert!(slab.slots().vacant_entry_in(2..4).is_none());
    assert!(slab.slots().vacant_entry_in(5..5).is_none());
    assert_eq!(slab.slots().vacant_entry_in(5..6).unwrap().key().index(), 5);
...
fn occupied_entry_carries_exclusive_generation_proof_through_removal() {
    let mut slab = Exclusive::<u32>::with_capacity(Capacity::new(2));
    ...
    let entry = slab
        .slots()
        .occupied_entry_at(key.index())
        .expect("occupied index");
...
fn indexed_entry_reserves_before_construction_and_rolls_back_on_drop() {
    let mut slab = Exclusive::<u32>::with_capacity(Capacity::new(4));
    let occupied = slab.slots().vacant_entry_at(1).unwrap().insert(7);
    assert!(slab.slots().vacant_entry_at(occupied.index()).is_none());
    assert!(slab.slots().vacant_entry_at(4).is_none());

    let reservation = slab.slots().vacant_entry_at(3).unwrap();
    let cancelled = reservation.key();
    assert_eq!(cancelled.index(), 3);
    drop(reservation);

    let replacement = slab.slots().vacant_entry_at(3).unwrap().insert(9);
...
fn sparse_iteration_clear_and_index_removal_follow_live_entries() {
    let mut slab: Exclusive<u32> = Exclusive::with_capacity(Capacity::new(128));
    assert_eq!(slab.slots().key(127), None);
    let high = slab.slots().vacant_entry_at(127).unwrap().insert(7);
    let low = slab.slots().vacant_entry_at(1).unwrap().insert(3);
    assert_eq!(slab.slots().values().copied().collect::<Vec<_>>(), [7, 3]);
    assert_eq!(slab.remove(high), Some(7));
    assert_eq!(slab.slots().values().copied().collect::<Vec<_>>(), [3]);
    ...
    slab.clear();
    assert!(slab.is_empty());
    assert_eq!(slab.slots().key(live.index()), None);
}
...
fn external_parts_resolve_only_the_current_generation() {
    ...
    let mut slab = Exclusive::<u32, Tag, MAX>::with_capacity(Capacity::new(1));
    let key = slab.insert(7).unwrap();
    let parts = Parts::<MAX>::new(key.index(), key.generation().get()).unwrap();
    assert_eq!(slab.slots().parts(parts), Some(&7));
    assert_eq!(slab.slots().remove_parts(parts), Some(7));
    assert_eq!(slab.slots().parts(parts), None);

    let slab = Cell::<u32, Tag, MAX>::with_capacity(Capacity::new(1));
    let stale = slab.insert(7).unwrap().parts();
    assert_eq!(slab.slots().remove_parts(stale), Some(7));
    let current = slab.insert(11).unwrap().parts();
    assert_eq!(slab.slots().update_parts(stale, |value| *value += 1), None);
    assert_eq!(
        slab.slots().update_parts(current, |value| *value += 1),
        Some(())
    );
    assert_eq!(slab.slots().remove_parts(current), Some(12));
}
