...
impl<V> Map<V> {
    ...
    pub fn try_from_plan(plan: Plan<V>) -> Result<Self, collections::AllocationError> {
        use crate::ThreadBound;
        let capacity = plan.capacity();
        let buckets = (capacity * 2).next_power_of_two();
        Ok(Self {
            controls: collections::BoxSliceExt::try_box_with(buckets, |_| EMPTY)?,
            hashes: collections::BoxUninitExt::try_box_uninit(buckets)?,
            values: collections::BoxUninitExt::try_box_uninit(buckets)?,
            len: 0,
            capacity,
...
impl<V> super::raw::Map<V> for Map<V> {
    unsafe fn entry_unchecked<'map>(
        ...
        match self.probe(hash, matches) {
            Probe::Occupied(index) => Entry::Occupied(Occupied { map: self, index }),
            Probe::Vacant(index) => Entry::Vacant(Vacant {
                map: self,
                index,
                hash,
            }),
            Probe::Full => unsafe { hint::unreachable_unchecked() },
        }
    }
    ...
    unsafe fn occupied_entry_unchecked<'map>(
        ...
        match self.probe(hash, matches) {
            Probe::Occupied(index) => Occupied { map: self, index },
            Probe::Vacant(_) | Probe::Full => unsafe { hint::unreachable_unchecked() },
        }
    }
}
