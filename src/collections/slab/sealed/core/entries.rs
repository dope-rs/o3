use crate::collections::slab::sealed::{
    self,
    core::{self, guards},
};

#[derive(Clone, Copy)]
pub(in crate::collections::slab::sealed) struct Entries<
    'a,
    T,
...
impl<'a, T, G: sealed::GenerationState, M: core::Mode, const PARTITIONS: usize>
    Entries<'a, T, G, M, PARTITIONS>
{
    pub(super) fn new(core: &'a core::Core<T, G, M, PARTITIONS>) -> Self {
        Self { core }
    }

    pub(in crate::collections::slab::sealed) fn get(
        self,
        index: u32,
        generation: G,
    ) -> Option<&'a T> {
    ...
    pub(in crate::collections::slab::sealed) fn remove(
        self,
        index: u32,
        generation: G,
    ...
    pub(in crate::collections::slab::sealed) fn remove_with<R>(
        self,
        index: u32,
        generation: G,
    ...
    pub(in crate::collections::slab::sealed) fn remove_index(
        self,
        index: u32,
    ) -> Option<(T, core::SlotIndex)> {
    ...
    pub(in crate::collections::slab::sealed) fn remove_index_with<R>(
        self,
        index: u32,
    ...
    pub(in crate::collections::slab::sealed) fn index(self, index: u32) -> Option<(&'a T, G)> {
        let slot = self.core.slots.get(index as usize)?;
    ...
    pub(in crate::collections::slab::sealed) fn generation(self, index: u32) -> Option<G> {
        let slot = self.core.slots.get(index as usize)?;
        (slot.state.get() == core::State::Occupied).then(|| slot.generation.get())
    }

    pub(in crate::collections::slab::sealed) fn occupied_at(
        self,
        position: usize,
    ) -> Option<(u32, G)> {
    ...
    pub(in crate::collections::slab::sealed) fn values(self) -> impl Iterator<Item = &'a T> + 'a {
    ...
    pub(in crate::collections::slab::sealed) fn update<R>(
        self,
        index: u32,
        generation: G,
    ...
    pub(in crate::collections::slab::sealed) fn update_index<R>(
        self,
        index: u32,
        f: impl FnOnce(&mut T) -> R,
    ) -> Option<R> {
