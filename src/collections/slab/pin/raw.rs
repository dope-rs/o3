...
use crate::collections::{
    self,
    slab::{self, GenerationState as _, key},
};
...
impl<T, const MAX: u32> Lazy<T, MAX> {
    pub(super) fn try_with_capacity(
        ...
        Ok(Self {
            slots: collections::try_box_uninit(capacity.get())?,
        })
    }
}
...
pub(super) struct Core<T, Tag, S: Slots<T, MAX>, const MAX: u32> {
    slots: S,
    free: u32,
    initialized: u32,
    len: usize,
...
pub(super) struct VacantEntry<'a, T, Tag, S: Slots<T, MAX>, const MAX: u32> {
    core: &'a mut Core<T, Tag, S, MAX>,
    index: u32,
}

impl<T, Tag, S: Slots<T, MAX>, const MAX: u32> VacantEntry<'_, T, Tag, S, MAX> {
    pub(super) fn insert(self, value: T) -> key::Key<Tag, MAX> {
        let slot = unsafe {
            self.core
                .slots
...
impl<T, Tag, S: Slots<T, MAX>, const MAX: u32> Core<T, Tag, S, MAX> {
    ...
    pub(super) fn insert(&mut self, value: T) -> Result<key::Key<Tag, MAX>, T> {
    ...
    pub(super) fn vacant_entry(&mut self) -> Option<VacantEntry<'_, T, Tag, S, MAX>> {
        if self.free == NONE {
    ...
    pub(super) fn get_index_mut(
        &mut self,
        index: u32,
    ) -> Option<(key::Key<Tag, MAX>, pin::Pin<&mut T>)> {
        let slot = self
            .slots
            .initialized_mut(self.initialized)
        ...
        let key = key::Key::new(index, slot.generation);
    ...
    pub(super) fn key(&self, index: u32) -> Option<key::Key<Tag, MAX>> {
        let slot = self
            .slots
            .initialized(self.initialized)
            .get(index as usize)?;
