...
/// Shared surplus for independently stored lane credit states.
pub struct FairCreditPool<const N: usize = 1> {
    shared: Cell<[usize; N]>,
    _thread: ThreadBound,
}

/// One lane's protected reserve and current holdings.
pub struct FairCreditState<const N: usize = 1> {
    reserve: [usize; N],
    held: Cell<[usize; N]>,
}
...
#[derive(Clone, Copy)]
pub struct FairCreditLane<'a, const N: usize = 1> {
...
pub struct FairCredits<const N: usize = 1> {
    used: Cell<[usize; N]>,
    pool: FairCreditPool<N>,
    held: Box<[Cell<[usize; N]>]>,
    reserve: [usize; N],
}

impl<const N: usize> FairCreditPool<N> {
    ...
    pub fn bind<'a>(&'a self, state: &'a FairCreditState<N>) -> FairCreditLane<'a, N> {
        FairCreditLane {
            shared: &self.shared,
            held: &state.held,
    ...
    pub fn shared(&self) -> [usize; N] {
        self.shared.get()
    }
}

impl<const N: usize> FairCreditState<N> {
    ...
    pub fn split(total: [usize; N], lane_count: usize, lane: usize) -> Option<Self> {
        if lane_count == 0 || lane >= lane_count {
            return None;
        }
        Some(Self::split_at(total, lane_count, lane))
    }
    ...
    pub fn held(&self) -> [usize; N] {
        self.held.get()
    }

    pub fn reserved(&self) -> [usize; N] {
        self.reserve
    }
}

impl<const N: usize> FairCreditLane<'_, N> {
    pub fn can_acquire_all(self, amount: [usize; N]) -> bool {
...
impl<const N: usize> FairCredits<N> {
    ...
    fn with_reserve_per_lane(capacity: [usize; N], lane_count: usize, reserve: [usize; N]) -> Self {
        ...
        Self {
            used: Cell::new([0; N]),
            pool: FairCreditPool::new(array::from_fn(|dimension| {
    ...
    fn credit(&self, lane: usize) -> Option<FairCreditLane<'_, N>> {
        Some(FairCreditLane {
    ...
    pub fn held_all_by(&self, lane: usize) -> Option<[usize; N]> {
        self.held.get(lane).map(Cell::get)
    }

    pub fn reserved_all_for(&self, lane: usize) -> Option<[usize; N]> {
